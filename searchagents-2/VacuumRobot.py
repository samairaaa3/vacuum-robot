...
class Gui(VacuumEnvironment):
    ...
    def setupUI(self):
        self.ExploredCount_label = Label(topframe, text='ExploredCount: 0', bg='green', fg='beige', padx=10, pady=5)
        self.ExploredCount_label.pack(side='left')
        self.PathCount_label = Label(topframe, text='PathCount: 0', bg='orange', fg='black', padx=10, pady=5)
        self.PathCount_label.pack(side='right')

        self.exit_button = Button(frame, text='Exit', bg='purple', fg='white', height=2, width=5, padx=2, pady=2)
    ...
    def setSearchEngine(self, choice):
        ...
        self.planner = SearchPlanning(self)
    ...
    def add_agent(self, agt, loc):
        ...
        self.add_thing(agt, loc)  #add the agent to env's list of agents
        assert(len(self.agents) == 1)
        # Place the agent at the provided location in GUI.
        lbl = agent_label(agt)
        self.buttons[loc[1]][loc[0]].config(bg='white', text=lbl, state='normal')
        self.agent = agt # set active agent. This is useful when we have multi-agent environment
    ...
    def execute_action(self, agent, action):
        ...
        if action == 'Suck':
            ...
            agent.location = self.planner.result(agent.location, action)
            self.buttons[yi][xi].config(text='')
    ...
    def step(self):
        """updates the environment one step.
        """
        if env.dirtCount == 0:
        ...
        if len(self.solution) == 0: # agent has reached a dirty room. So the proper action is 'suck'
            ...
            self.read_env()
            if env.dirtCount > 0 and self.planner is not None:
                self.planner.generateNextSolution()
                #self.running = False
    ...
    def exit(self):
        """Exiting the app. Do any save history or such here."""
        self.reset_env()
        exit(0)
    ...
    def reset_env(self):
        ...
        self.running = False
        self.ExploredCount_label.config(text="ExploredCount: " + str(0))
        self.PathCount_label.config(text="PathCount: " + str(0))
        self.exploredCount = 0
        self.pathCount = 0
        #self.searchTypeStr.set(args['searchType'])
...
def readCommand( argv ):
    """
    Processes the command used to run vacuumRobot from the command line.
    """
    from optparse import OptionParser
    usageStr = """
    USAGE:      python VacuumRobot.py <options>
    EXAMPLES:   (1) python VacuumRobot.py
                    - starts an interactive game
                (2) python VacuumRobot.py -s A*
                    -perform A* search to clean all rooms
                (3)  python VacuumRobot.py -s Greedy -r Manhattan
                    - perform greedy algorithm search with Manhattan cost function
                (4) python VacuumRobot.py -s UCS -c StayUp
    """
    ...
    parser.add_option('-e', '--earlyTest', action='store_true', dest='early',
                      help='Apply early goal test (when pushing on Frontier vs when popping)', default=False)
...
if __name__ == "__main__":
    """ 
    The main function called when run from command line: >python VacuumRobot.py
    """
    args = readCommand(sys.argv[1:])  # Get game components based on input

    win = Tk()
    win.title("Cleaning Robot")
